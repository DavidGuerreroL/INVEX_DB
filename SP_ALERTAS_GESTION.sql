...
BEGIN
 ...
 IF V_BANDERA = 1
  Then /*CREAR ALERTA */
	...
	select count(*) into v_Exists from cat_logins where ltrim(rtrim(CAT_LO_NOMBRE)) 
 = ltrim(rtrim(V_USUARIO));
    ...
    Systimestamp,
    CASE WHEN NVL(V_VIGENCIA,'') ='' THEN sysdate+5 ELSE cast(to_char(V_VIGENCIA
, 'dd/mm/yyyy' )as date) END,
    NVL((SELECT MAX(HIST_AL_ID)+1 FROM HIST_ALERTAS_GESTION WHERE HIST_AL_USUARI
O=V_USUARIO),0),
    V_MENSAJE,
    V_TIPO,
    V_USUARIO2
     ...
     open CV_1 for SELECT 'OK' msg FROM dual ;   -- si se agregan columans de sa
lida se debe validar en add hist gest y add hist promesas la parte de alertas
ELSIF V_BANDERA = 2 THEN /*ACTUALIZAR FECHA VISTO ALERTA */
    UPDATE HIST_ALERTAS_GESTION SET HIST_AL_DTEVISTO = SYSDATE WHERE HIST_AL_DTE
VISTO is null AND HIST_AL_USUARIO = V_USUARIO;
    COMMIT;
...
O = V_USUARIO AND HIST_AL_DTEVISTO IS NULL;
    ...
    open cv_1 for select HIST_AL_DTECREADA FECHACREACION, HIST_AL_DTEVISTO FECHA
LECTURA, HIST_AL_MENSAJE ALERTA, HIST_AL_DTEVIGENCIA FECHAVIGENCIA from HIST_ALE
RTAS_GESTION
    WHERE HIST_AL_USUARIO=V_USUARIO and case when V_TIPO='0' then trunc(HIST_AL_
DTECREADA) when V_TIPO='1' then trunc(HIST_AL_DTEVISTO) when V_TIPO='2' then tru
nc(HIST_AL_DTEVIGENCIA) end
    between to_date(V_MENSAJE,'rrrr-mm-dd-hh24-mi-ss') and to_date(V_VIGENCIA,'r
rrr-mm-dd-hh24-mi-ss'); --2019-08-26-00-00-00
  elsif v_bandera=6 then
    update HIST_ALERTAS_GESTION set HIST_AL_DTEVISTO=sysdate where HIST_AL_USUAR
IO=V_USUARIO and case when V_TIPO=0 then HIST_AL_DTECREADA when V_TIPO=1 then HI
ST_AL_DTEVISTO when V_TIPO=2 then HIST_AL_DTEVIGENCIA end between to_date(V_MENS
AJE,'dd/mm/rr')
    and to_date(V_VIGENCIA,'dd/mm/rr') and HIST_AL_DTEVISTO is null;
    commit;
    open cv_1 for select 'Ok' MSJ from dual;
  elsif v_bandera=7 then
    open cv_1 for select distinct HIST_AL_TIPO tipo from HIST_ALERTAS_GESTION wh
ere HIST_AL_USUARIO=V_USUARIO;
  end if;
